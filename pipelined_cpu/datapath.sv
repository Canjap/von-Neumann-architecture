...
module datapath (
    ...
    output logic        lrwriteW_out,  // SETLR in WB — needed by hazard lrstall
    ...
    // Flush on: reset, branch taken (only when not stalled), or hazard flushD.
    // Stall on: stallD (holds current instruction).
    // When stallD=1 the flush is suppressed: branch_taken can be a false
    // positive because the acc register and the branch evaluator update at
    // the same posedge.  The stall already holds IF/ID; suppressing the flush
    // lets the stall release first, after which branch_taken re-evaluates
    // with the freshly-written ACC value.
    // =========================================================
    logic [31:0] instrD, pcplus4D;

    always_ff @(posedge clk or posedge reset) begin
        if (reset || (branch_taken && !stallD) || flushD) begin
            instrD   <= 32'b0;
            pcplus4D <= 32'b0;
    ...
    // WB→ID bypass: when WB is writing ACC at the same posedge an instruction
    // transitions from ID to EX, the ID/EX latch would otherwise capture the
    // stale pre-write accD value.  Forward resultW so accE is always current.
    ...
    always @(posedge clk) begin
        if (branch_taken && !stallD && !reset) begin
    ...
    always_ff @(posedge clk or posedge reset) begin
        if (reset || flushE) begin
            ...
            accE         <= accD_bypassed; signimmE     <= signimmD;
